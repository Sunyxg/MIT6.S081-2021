  ...
  for(int i = 0; i < 512; i++){
    ...
    if(pte & PTE_V){
      ...
      if((pte & (PTE_R|PTE_W|PTE_X))==0){
        ...
        k_freepagetable((pagetable_t)child);
      }
    }
  }
...
void
uvminithart(pagetable_t k_pagetable)
{
...
int
pgtable_copy(pagetable_t proc_pagetable, pagetable_t k_pagetable, uint64 oldsz, uint64 newsz)
{
  pte_t *pte_p, *pte_k;
  uint64 pa, i;
  uint flags;
  ...
  for(i = oldsz; i < newsz; i += PGSIZE){
    if((pte_p = walk(proc_pagetable, i, 0)) == 0)
      panic("pgcopy: proc_pte should exist");

    pa = PTE2PA(*pte_p);
    flags = PTE_FLAGS(*pte_p);
    flags = (flags & (~PTE_U));

    if((pte_k = walk(k_pagetable, i, 1)) == 0)
      panic("pgcopy: k_pte should exist");
